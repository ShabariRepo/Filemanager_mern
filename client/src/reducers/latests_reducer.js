      ...
      if (exists !== undefined) {
        ...
        if (exists.revisions < 2) {
          ...
          if (exists.latestName === action.payload.name) {
            console.log(
              `Deleting latest version, override current`
            );
            latestFlag = true;
          }
          ...
          if(exists.versions.includes(action.payload.name)){
            ...
            if(latestFlag){
              ...
              console.log(`Deleting latest version ${action.payload.name} and moving previous up ${exists.versions[prior]}`);
            }
          } else{
            console.log(`this file did not exist in the versions array ${action.payload.name}`);
            return;
          }
